...
const StockOutCountdown = React.memo(
    ({
        current,
        onOrder,
        dailyDemand,
        ...
        return (
            <div className="flex flex-col items-center">
                <Typography variant="body" className={`font-bold ${color}`}>
                    {daysLeft}d
                </Typography>
                <Typography variant="label" className="text-slate-500 !font-semibold normal-case">
                    {mos.toFixed(1)} MOS
                </Typography>
            </div>
        );
...
export const ExecutiveDashboard = React.memo(
    ({
        ...
        const currencyFormatter = useMemo(
            () =>
                new Intl.NumberFormat(settings.costBasis === 'PP' ? 'vi-VN' : 'en-IE', {
                    style: 'currency',
                    currency: settings.costBasis === 'PP' ? 'VND' : 'EUR',
                    notation: 'compact',
                }),
            [settings.costBasis],
        );
            ...
            <div className="flex flex-col bg-white/90 backdrop-blur-md rounded-3xl border border-white/20 overflow-hidden w-full shadow-glass hover:shadow-xl transition-all duration-500">
                <div className="px-6 py-4 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center bg-slate-50/30 sticky top-0 z-40 gap-4">
                    <div className="flex items-center gap-4">
                        <Typography
                            variant="label"
                            className="bg-white text-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm"
                        >
                            {t('db_inventory_grid')}
                        </Typography>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {!isMobile && (
                            <button
                                onClick={() => exportToCSV(sortedList, 'export.csv', exportOptions)}
                                className="bg-white text-slate-700 hover:bg-slate-50 hover:text-blue-600 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border border-slate-200 transition-all flex items-center gap-2 shadow-sm"
                            >
                ...
                {isMobile ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4 stagger-children">
                        {paginatedData.map(item => {
                            ...
                            const draftQty = draftData?.quantities?.[item.ItemCode]
                                ? draftData.quantities[item.ItemCode].air + draftData.quantities[item.ItemCode].sea
                                : 0;
                            ...
                            return (
                                <div
                                    key={item.ItemCode}
                                    className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm active:scale-[0.98] transition-all hover:border-blue-300 hover:shadow-md relative overflow-hidden"
                                >
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-1 flex flex-wrap items-center gap-2">
                                                ...
                                                {item.TypeCar && (
                                                    <Typography
                                                        variant="label"
                                                        className="bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded"
                                                    >
                                                        {item.TypeCar}
                                                    </Typography>
                                                )}
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-4">
                                        <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                                            <Typography
                                                variant="label"
                                                className="text-slate-400 block mb-1 uppercase tracking-tighter"
                                            >
                                                DEMAND
                                            </Typography>
                                        ...
                                        <div className="bg-blue-50/50 p-2 rounded-xl border border-blue-100">
                                            <Typography
                                                variant="label"
                                                className="text-blue-400 block mb-1 uppercase tracking-tighter"
                                            >
                                                Pipeline (In/PO)
                                            </Typography>
                                            <div className="flex items-center gap-2 tabular-nums">
                                                ...
                                                <span className="font-black text-indigo-600">
                                                    PO: {item.TotalPO.toLocaleString()}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-between items-end pt-2 border-t border-slate-50">
                                            <div className="flex flex-col">
                                                ...
                                                <Typography
                                                    variant="h3"
                                                    className={`tabular-nums ${(item.computed?.mos || 0) < 1 ? 'text-rose-600' : 'text-slate-900'}`}
                                                >
                                                    {demandMonthly <= 0
                                                        ? '∞'
                                                        : `${(item.computed?.mos || 0).toFixed(1)} M`}
                                                </Typography>
                                            </div>
                                            <div className="flex flex-col text-right">
                                                ...
                                                >
                                                    {currencyFormatter.format(item.computed?.stockValue || 0)}
                                                </Typography>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="overflow-x-auto relative custom-scrollbar">
                        <table className="w-full text-sm text-left border-separate border-spacing-0 min-w-[1600px] table-zebra">
                            <thead className="bg-gradient-header text-slate-500 sticky top-0 z-30 font-black uppercase text-xs tracking-wider border-b-2 border-slate-200 backdrop-blur-sm shadow-sm transition-all">
                                <tr>
                                    <th className="px-4 py-4 w-12 text-center border-b border-slate-200/60 sticky left-0 z-40 bg-white">
                                        #
                                    </th>
                                    <th className="px-6 py-4 min-w-[220px] sticky left-12 z-40 bg-white border-b border-slate-200/60 sticky-column-shadow">
                                        SKU IDENTITY
                                    </th>
                                    <th className="px-4 py-4 border-b border-slate-200/60 text-center min-w-[110px]">
                                        DEMAND
                                    </th>
                                    <th className="px-4 py-4 border-b border-slate-200/60 text-right min-w-[145px]">
                                        STOCK HEALTH
                                    </th>
                                    <th className="px-4 py-4 border-b border-slate-200/60 text-center min-w-[110px]">
                                        SUPPLY PIPELINE
                                    </th>
                                    <th className="px-4 py-4 text-center border-b border-slate-200/60 min-w-[120px]">
                                        {t('ord_th_momentum')}
                                    </th>
                                    <th className="px-4 py-4 text-center border-b border-slate-200/60 min-w-[80px]">
                                        RUNWAY
                                    </th>
                                    <th className="px-4 py-4 text-center border-b border-slate-200/60">
                                        {t('ord_th_dealer_cst')}
                                    </th>
                                    <th className="px-4 py-4 text-right border-b border-slate-200/60">
                                        {t('th_stock_val')}
                                    </th>
                                    <th className="px-4 py-4 text-center sticky right-0 z-40 bg-white border-b border-slate-200/60 border-l border-slate-200">
                                        <FaIcon className="fas fa-gear" />
                                    </th>
                                </tr>
                            </thead>
                            <tbody className="bg-white stagger-children">
                                {paginatedData.map((item, idx) => {
                                    const m1Actual = item.SalesHistory
                                    ...
                                    const currentCst =
                                        demandMonthly > 0
                                            ? (item.NetDemand + item.DealerInventory) / demandMonthly
                                            : 99.9;
                                    const draftQty = draftData?.quantities?.[item.ItemCode]
                                        ? draftData.quantities[item.ItemCode].air +
                                          draftData.quantities[item.ItemCode].sea
                                        : 0;
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-4 py-3 text-center text-xs font-black text-slate-400 font-mono border-b border-slate-50 sticky left-0 z-10 bg-white group-hover:bg-slate-50/80">
                                            ...
                                            <td className="px-4 py-3 text-center border-b border-slate-50">
                                                <div className="flex flex-col items-center gap-1">
                                                    <div
                                                        className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-lg border border-slate-200/80 shadow-sm tabular-nums"
                                                ...
                                                <div className="flex flex-col items-center gap-1">
                                                    {incomingThisMonth > 0 ? (
                                                        <div className="flex flex-col items-center tabular-nums">
                                                            ...
                                                            <Typography
                                                                variant="label"
                                                                className="text-blue-400 !font-semibold normal-case leading-tight"
                                                            >
                                                                Về tháng này
                                                            </Typography>
                                                        </div>
                                            ...
                                            <td className="px-4 py-3 text-center border-b border-slate-50">
                                                {demandMonthly <= 0 ? (
                                                    <div className="flex flex-col items-center">
                                                        ...
                                                        <Typography
                                                            variant="label"
                                                            className="text-slate-400 !font-semibold normal-case"
                                                        >
                                                            No demand
                                                        </Typography>
                                                    </div>
                                                    ...
                                                    <StockOutCountdown
                                                        current={item.computed?.available || 0}
                                                        onOrder={item.TotalPO}
                                                        dailyDemand={item.computed?.demandRateDaily || 0}
                                                        backorder={item.Backorder}
                                                    />
                                                )}
                                            </td>
                                            <td className="px-4 py-3 text-center border-b border-slate-50">
                                                <div className="flex flex-col items-center">
                                                    <DealerStockPopup items={item.DealerBreakdown || []}>
                                                        <span
                                                            className={`font-bold tabular-nums text-[13px] cursor-help hover:scale-105 transition-transform inline-flex items-center gap-1 ${(item.DealerBreakdown?.length || 0) > 0 ? 'text-blue-700' : 'text-slate-800'}`}
                                                        >
                                                    ...
                                                    <div className="mt-1">
                                                        <div
                                                            className={`text-sm font-black px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200 tabular-nums`}
                                                        >
                                                            {demandMonthly <= 0
                                                                ? 'CST: ∞'
                                                                : `CST: ${currentCst.toFixed(1)}`}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-4 py-3 text-center sticky right-0 z-40 bg-white group-hover:bg-slate-50/80 border-b border-slate-50 border-l border-slate-200">
                                                <FaIcon className="fas fa-chevron-right text-slate-300 group-hover:text-blue-500 transition-colors" />
                                            </td>
                                        </tr>
                                    );
                                })}
                ...
                <div className="px-4 sm:px-8 py-4 border-t-2 border-slate-200 bg-white flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center justify-between w-full sm:w-auto gap-4 text-slate-600">
                        <select
                            ...
                            className="bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5 outline-none cursor-pointer text-slate-700 font-bold text-xs sm:text-sm"
                        >
                        ...
                        <Typography variant="label" className="text-slate-400 whitespace-nowrap">
                            {t('common_total')}:{' '}
                            <Typography as="span" variant="label" className="text-slate-700 tabular-nums">
                                {sortedList.length}
                            </Typography>
                        </Typography>
                    </div>
                        ...
                        {[...Array(totalPages)].map((_, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`pagination-pill shrink-0 tabular-nums ${currentPage === page ? 'active' : 'text-slate-600'}`}
                                >
                                    {page}
                                </button>
                            );
