    ...
    return (
        <div className="space-y-2">
            <div className="flex items-center justify-between">
                <Typography variant="label" className="text-slate-500 uppercase tracking-wider">
                    {label}
                </Typography>
            ...
            {scenario !== undefined && (
                <div className="flex items-center gap-3">
                    ...
                    <Typography
                        variant="h3"
                        className={`min-w-[70px] text-right ${isIncrease ? 'text-orange-600' : 'text-emerald-600'}`}
                    >
                        {scenario.toLocaleString()}
    ...
    const handleSmartForecast = async () => {
        ...
        try {
            ...
            setAIResult({
                ...
                confidence: 'MEDIUM',
                trend_analysis: 'Sử dụng weighted average (Fallback)',
            });
        } finally {
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 pl-4 border-l border-slate-100">
                        <div className="flex flex-col">
                            <Typography variant="label" className="text-slate-400 !text-[9px]">
                                STOCK
                            </Typography>
                            <Typography variant="mono-sm" className="font-bold text-slate-700">
                        ...
                        <div className="flex flex-col">
                            ...
                            <Typography variant="mono-sm" className="font-bold text-blue-600">
                                {(item.TotalPO || 0).toLocaleString()}
                            </Typography>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <div className="lg-segmented scale-90 origin-right ml-1">
                        <button
                            ...
                            className={mode === 'BASIC' ? 'lg-active' : ''}
                        >
                            {t('sim_basic') || 'CƠ BẢN'}
                        </button>
                        <button
                            ...
                            className={mode === 'SCENARIO' ? 'lg-active' : ''}
                        >
                            {t('sim_scenario') || 'KỊCH BẢN'}
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                            <div>
                                ...
                                <input
                                    ...
                                    max="100"
                                    step="5"
                                    value={scenarioDemandPct}
                                    onChange={e => setScenarioDemandPct(Number(e.target.value))}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    step="5"
                                    value={scenarioLTPct}
                                    onChange={e => setScenarioLTPct(Number(e.target.value))}
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <div className="relative group">
                                    <Typography
                                        variant="label"
                                        className="text-slate-500 uppercase tracking-tight mb-1 ml-1 block !text-[9px]"
                                    >
                                        Demand / Month
                                    </Typography>
                                    <input
                                        type="number"
                                        value={simDemand}
                                        onChange={e => setSimDemand(Math.max(0, Number(e.target.value)))}
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-1.5 text-sm font-bold text-slate-800 outline-none focus:bg-white focus:border-blue-400 transition-all shadow-inner"
                                    />
                                </div>
                                <div className="relative group">
                                    <Typography
                                    ...
                                    <input
                                        type="number"
                                        value={simLT}
                                        onChange={e => setSimLT(Math.max(1, Number(e.target.value)))}
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-1.5 text-sm font-bold text-slate-800 outline-none focus:bg-white focus:border-blue-400 transition-all shadow-inner"
                                    />
                                </div>
                                <div className="relative group">
                                    ...
                                    <input
                                        type="number"
                                        value={simSP}
                                        onChange={e => setSimSP(Math.max(1, Number(e.target.value)))}
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-1.5 text-sm font-bold text-slate-800 outline-none focus:bg-white focus:border-indigo-400 transition-all shadow-inner"
                                    />
                                </div>
                                <div className="relative group">
                                    <Typography
                                    ...
                                    <input
                                        type="number"
                                        value={simSSP}
                                        onChange={e => setSimSSP(Math.max(0, Number(e.target.value)))}
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-1.5 text-sm font-bold text-slate-800 outline-none focus:bg-white focus:border-emerald-400 transition-all shadow-inner"
                                    />
                                </div>
                            </div>
                            <button
                                ...
                                className="w-full h-10 bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-xl font-bold text-[9px] uppercase tracking-[0.1em] flex items-center justify-center gap-2 hover:shadow-glass hover:scale-[1.01] transition-all disabled:opacity-60 border border-emerald-400/30"
                            >
                                {isForecasting ? (
                                    <FaIcon className="fas fa-circle-notch animate-spin" />
                                ) : (
                                    <FaIcon className="fas fa-robot text-sm" />
                                )}
                    ...
                    {aiResult && (
                        <div className="bg-gradient-to-br from-emerald-50/30 to-teal-50/30 backdrop-blur-sm rounded-2xl p-4 border border-emerald-100/50 flex items-start gap-4 animate-fadeIn relative overflow-hidden shadow-sm">
                            <div className="w-10 h-10 bg-white/80 text-emerald-600 rounded-xl flex items-center justify-center shadow-soft border border-emerald-50 shrink-0 text-lg">
                                🤖
                            </div>
                            <div className="relative z-10 flex-1">
                                <div className="flex justify-between items-center mb-1">
                                    <Typography variant="h3" className="text-emerald-800 tracking-tight !text-[10px]">
                                        AI Insight
                                    </Typography>
                                    <Typography
                                        variant="mono-sm"
                                        className="bg-emerald-100/50 text-emerald-700 px-1.5 py-0.5 rounded-lg border border-emerald-200 !text-[9px] font-bold"
                                    >
                                ...
                                <Typography
                                    variant="body-sm"
                                    className="text-emerald-800/80 leading-relaxed font-medium !text-xs italic"
                                >
                                    "{aiResult.trend_analysis.substring(0, 100)}..."
                                </Typography>
                            </div>
                        </div>
                    )}
                ...
                <div className="space-y-4">
                    <div className="bg-slate-50/30 rounded-2xl border border-slate-100/50 p-4 space-y-4">
                        <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                            <Typography
                                variant="label"
                                className="text-slate-400 uppercase tracking-widest flex items-center gap-2 !text-[10px]"
                            >
                                <FaIcon className="fas fa-calculator text-blue-400" /> Results
                            </Typography>
                            {baseline.stockoutRisk && (
                                <Typography
                                    variant="mono-sm"
                                    className="bg-rose-50 text-rose-600 px-2 py-0.5 rounded-lg font-bold border border-rose-100 shadow-sm animate-pulse !text-[9px]"
                                >
                                    RISK
                                </Typography>
                            )}
                        ...
                        <div className="space-y-3">
                            <MetricBar
                                label="Safety Stock"
                                baseline={baseline.safetyStock}
                                scenario={mode === 'SCENARIO' ? scenario.safetyStock : undefined}
                            />
                            <MetricBar
                                label="Reorder Point"
                                baseline={baseline.rop}
                                scenario={mode === 'SCENARIO' ? scenario.rop : undefined}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div
                            className={`rounded-2xl p-4 border transition-all flex flex-col justify-between h-24 relative overflow-hidden ${baseline.mos < 1 ? 'bg-rose-50/30 border-rose-100' : 'bg-slate-50/50 border-slate-100'}`}
                        >
                            <Typography
                                variant="label"
                                className="text-slate-400 uppercase tracking-widest flex items-center gap-2 !text-[9px]"
                            >
                            ...
                            <div className="flex items-baseline gap-1.5">
                                <Typography
                                    variant="h3"
                                    className={`${baseline.mos < 1 ? 'text-rose-600' : baseline.mos < 2 ? 'text-amber-500' : 'text-emerald-600'}`}
                                >
                                    {baseline.mos > 99 ? '∞' : baseline.mos.toFixed(1)}
                                </Typography>
                                <Typography
                                    variant="label"
                                    className="text-slate-400 font-bold uppercase !text-[8px] tracking-tighter"
                                >
                                    Mths
                                </Typography>
                            </div>
                        ...
                        <div
                            className={`rounded-2xl p-4 border transition-all flex flex-col justify-between h-24 relative overflow-hidden ${baseline.stockAtDelivery < 0 ? 'bg-rose-50/30 border-rose-100' : 'bg-slate-50/50 border-slate-100'}`}
                        >
                            <Typography
                                variant="label"
                                className="text-slate-400 uppercase tracking-widest flex items-center gap-2 !text-[9px]"
                            ...
                            <div className="flex items-baseline gap-1.5">
                                <Typography
                                    variant="h3"
                                    className={`${baseline.stockAtDelivery < 0 ? 'text-rose-600' : 'text-slate-700'}`}
                                >
                                    {baseline.stockAtDelivery > 0 ? '+' : ''}
                                    {Math.round(baseline.stockAtDelivery).toLocaleString()}
                                </Typography>
                                <Typography
                                    variant="label"
                                    className="text-slate-400 font-bold uppercase !text-[8px] tracking-tighter"
                        ...
                        <div className="rounded-2xl p-4 border bg-white/50 border-slate-100 shadow-glass-sm flex flex-col justify-between h-24 relative overflow-hidden group">
                            ...
                            <div className="flex items-baseline gap-1">
                                <Typography variant="h3" className="text-indigo-600 tracking-tighter">
                                    ...
                                    }).format(baseline.capitalRequired)}
                                </Typography>
                                <Typography
                                    variant="label"
                                    className="text-indigo-300 font-bold uppercase !text-[8px] tracking-tighter"
                                >
                                    VND
                                </Typography>
                            </div>
