                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="p-4 min-w-[280px] text-xs font-black text-slate-400 uppercase tracking-widest border-r border-slate-200">{t('kit_comparison_header')}</th>
                            {selectedSets.map((set, idx) => (
                                <th key={set.setCode} className={`p-4 min-w-[220px] border-r border-slate-200 last:border-r-0 ${idx === 0 ? 'bg-blue-50/30' : idx === 1 ? 'bg-amber-50/30' : 'bg-purple-50/30'}`}>
                                    <div className="text-xs font-black text-slate-400 uppercase mb-1">{t('kit_comparison_package')} {String.fromCharCode(65 + idx)}</div>
                                ...
                                <tr key={sku} className={`hover:bg-slate-50/80 transition-colors ${isCommon ? 'bg-emerald-50/20' : ''}`}>
                                    <td className="p-4 border-r border-slate-100">
                                        <div className="flex items-center gap-2">
                                            {isCommon && <div className="w-2 h-2 rounded-full bg-emerald-500" title={t('kit_common_part')}></div>}
                                                ...
                                                {c ? (
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex flex-col">
                                                            <span className="text-2xs font-bold text-slate-400 uppercase">{t('kit_comparison_required')}</span>
                                                            <span className="text-sm font-black text-slate-800">x{c.def.QtyRequired}</span>
                                                        </div>
                                                        <div className="flex flex-col text-right">
                                                            <span className="text-2xs font-bold text-slate-400 uppercase">{t('kit_comparison_stock')}</span>
                    ...
                    <tfoot className="bg-slate-50 font-black border-t-2 border-slate-200">
                        <tr>
                            <td className="p-4 text-xs text-slate-500 uppercase border-r border-slate-200">{t('kit_comparison_footer')}</td>
                            {selectedSets.map((set) => (
                                <td key={set.setCode} className="p-4 border-r border-slate-200 last:border-r-0">
                                    <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                            <span className="text-slate-400 uppercase">{t('kit_comparison_capital')}</span>
                                            <span className="text-blue-700">{(set.packageStockValue / 1000000).toFixed(1)}tr</span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                            <span className="text-slate-400 uppercase">{t('kit_comparison_available')}</span>
                                            <span className="text-emerald-600 font-black">{set.maxSetsNB + set.maxSetsBB} {t('kit_unit_sets')}</span>
                                        </div>
                                    </div>
                                </td>
                            ))}
...
export const RepairPackageOptimizer = ({
    ...
    const handleSaveToCloud = async () => {
        const pin = prompt(t('kit_prompt_pin'));
        if (pin === null) return;
        if (!verifyAdminPin(pin)) {
            alert(t('kit_alert_pin_wrong'));
            return;
        }
        ...
        if (success) {
            alert(t('kit_alert_save_ok'));
        } else {
            alert(t('kit_alert_save_fail'));
        }
    };
    ...
    const handleLoadFromCloud = async () => {
        ...
        if (cloudData && Array.isArray(cloudData)) {
            ...
            alert(t('kit_alert_load_ok'));
        } else {
            alert(t('kit_alert_load_fail'));
        }
    };
    ...
    const toggleSelection = (code: string) => {
        setSelectedComparisonCodes(prev => {
            ...
            if (prev.length >= 3) { alert("Max 3 packages for comparison."); return prev; }
            return [...prev, code];
        });
    };
    ...
    const handleAddToDraft = (set: any, region: 'NB' | 'BB') => {
        ...
        const messages = [];
        if (addedCount > 0) messages.push(`${addedCount} SKU to purchase`);
        if (transferCount > 0) messages.push(`${transferCount} SKU to transfer`);

        if (messages.length > 0) {
            onUpdateDraft({ quantities: currentQuantities, notes: currentNotes });
            alert(`Draft updated: ${messages.join(' & ')} for ${qtyToBuild} ${t('kit_unit_sets')} ${set.setName} (${region}).`);
        } else {
            alert(`Stock in ${region} is sufficient for ${qtyToBuild} ${t('kit_unit_sets')}! No action needed.`);
        }
    ...
    const handleClearDraft = () => {
        ...
        if (window.confirm(t('kit_confirm_clear'))) {
    ...
    if (!kittingDefs || kittingDefs.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] bg-white rounded-2xl border-2 border-dashed border-slate-300 p-8 text-center animate-fadeIn">
                ...
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-2">{t('kit_empty_no_data')}</h3>
                <p className="text-slate-500 max-w-md mb-8 text-base">{t('kit_empty_desc')}</p>
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden border border-white/5 shadow-premium">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-amber-200 text-sm font-bold mt-1 uppercase tracking-widest">{t('kit_subtitle')}</p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        {(totalDraftItems > 0 || hasInputs) && (
                            <div className="flex items-center gap-2 bg-white px-2 py-1.5 rounded-xl border border-slate-200">
                                <span className="text-[10px] font-black text-slate-400 uppercase pl-1">{t('common_selected')}: {totalDraftItems} SKU</span>
                                <button
                                ...
                                >
                                    <i className="fas fa-trash-alt mr-1"></i> {t('kit_btn_delete_draft')}
                                </button>
                            </div>
                        )}
                        ...
                        >
                            <i className={`fas ${isLoadingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-down'} text-blue-600`} /> {t('kit_btn_load_cloud')}
                        </button>
                        ...
                        {kittingDefs.length > 0 && (
                            <button
                                ...
                                className="bg-white text-slate-800 hover:bg-slate-50 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-sm border border-slate-200 disabled:opacity-60"
                            >
                                <i className={`fas ${isSavingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-up'} text-emerald-600`} /> {t('kit_btn_save_cloud')}
                            </button>
                        )}
                        ...
                        <div className="relative group w-56">
                            ...
                            <input type="text" placeholder={t('kit_search_ph')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl outline-none text-xs font-black text-slate-900 placeholder-slate-400 focus:ring-4 focus:ring-blue-500/5 transition-all" />
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div key={set.setCode} className={`bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition-all flex flex-col relative ${selectedComparisonCodes.includes(set.setCode) ? 'border-blue-500 ring-2 ring-blue-50 bg-blue-50/10' : 'border-slate-200'}`}>
                                ...
                                <div className="flex justify-between items-start mb-4 border-b border-slate-50 pb-4 pr-8">
                                    ...
                                    <div className="text-right">
                                        <div className="text-xs font-black text-slate-400 uppercase tracking-widest">{t('kit_metric_capital')}</div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                        <div className="flex justify-between items-baseline mb-2">
                                            <span className="text-xs font-black text-blue-700 uppercase">{t('kit_label_south')}</span>
                                            <div className="flex flex-col items-end">
                                                <span className="text-2xs font-bold text-slate-400 uppercase">{t('kit_label_feasible')}</span>
                                                <span className="text-2xl font-black text-slate-800 leading-none">{set.maxSetsNB} {t('kit_unit_sets')}</span>
                                            </div>
                                        </div>
                                        <SupplyChainStatusBar rate={set.completionRateNB} />
                                        <div className="mt-3 flex gap-1 items-center">
                                            <div className="relative w-full">
                                                <input type="number" min="0" placeholder="Qty" className="w-full h-8 pl-2 pr-8 text-center text-sm font-black bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-400 transition-colors" value={orderInputs[set.setCode]?.nb || ''} onChange={e => setOrderInputs(p => ({ ...p, [set.setCode]: { ...p[set.setCode], nb: parseInt(e.target.value) || 0 } }))} />
                                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-2xs text-slate-400 font-bold uppercase">{t('kit_unit_sets')}</span>
                                            </div>
                                            <button onClick={() => handleAddToDraft(set, 'NB')} className="bg-blue-600 text-white w-8 h-8 rounded-lg flex items-center justify-center shadow-sm active:scale-90 transition-all hover:bg-blue-700" title={t('kit_tooltip_add_nb')}><i className="fas fa-cart-plus text-xs"></i></button>
                                        </div>
                                    </div>
                                    {/* MIỀN BẮC REFILL */}
                                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                                        <div className="flex justify-between items-baseline mb-2">
                                            <span className="text-xs font-black text-amber-700 uppercase">{t('kit_label_north')}</span>
                                            <div className="flex flex-col items-end">
                                                <span className="text-2xs font-bold text-slate-400 uppercase">{t('kit_label_feasible')}</span>
                                                <span className="text-2xl font-black text-slate-800 leading-none">{set.maxSetsBB} {t('kit_unit_sets')}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-3 flex gap-1 items-center">
                                            ...
                                            <button onClick={() => handleAddToDraft(set, 'BB')} className="bg-blue-600 text-white w-8 h-8 rounded-lg flex items-center justify-center shadow-sm active:scale-90 transition-all hover:bg-blue-700" title={t('kit_tooltip_add_bb')}><i className="fas fa-cart-plus text-xs"></i></button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button onClick={() => setExpandedSet(expandedSet === set.setCode ? null : set.setCode)} className="mt-4 text-xs font-black text-slate-400 hover:text-blue-600 text-center uppercase tracking-[0.2em] transition-colors">
                                    {expandedSet === set.setCode ? t('kit_btn_collapse') : t('kit_btn_expand')}
                                </button>
                                        ...
                                        <table className="w-full text-xs text-left">
                                            <thead className="bg-slate-50 text-slate-400 font-black uppercase">
                                                <tr>
                                                    ...
                                                    <th className="px-2 py-1.5 text-center">Std Qty</th>
                                                    <th className="px-2 py-1.5 text-center bg-blue-50 text-blue-700">Need (Input)</th>
                                                    <th className="px-2 py-1.5 text-right">NB</th>
                                                    <th className="px-2 py-1.5 text-right">BB</th>
                                                    <th className="px-2 py-1.5 text-center bg-slate-100 text-slate-700">Solution</th>
                                                </tr>
                                            </thead>
                                                            ...
                                                            <td className="px-2 py-1.5 text-center bg-slate-50/50 border-x border-slate-100">
                                                                {totalNeeded > 0 ? (
                                                                    <div className="flex justify-center gap-2 text-2xs font-bold">
                                                                        {totalStockUse > 0 && <span className="text-emerald-600 bg-emerald-50 px-1.5 rounded border border-emerald-100" title={t('kit_solution_stock')}>✅ {totalStockUse}</span>}
                                                                        {totalTransfer > 0 && <span className="text-purple-600 bg-purple-50 px-1.5 rounded border border-purple-100" title={`${t('kit_solution_transfer')}: ${transferToNB > 0 ? 'BB->NB' : 'NB->BB'}`}>🔄 {totalTransfer}</span>}
                                                                        {totalBuy > 0 && <span className="text-rose-600 bg-rose-50 px-1.5 rounded border border-rose-100" title={t('kit_solution_purchase')}>🚢 {totalBuy}</span>}
                                                                    </div>
                ...
                <div className="space-y-8 animate-fadeIn">
                    ...
                    {selectedComparisonCodes.length >= 2 ? (
                        <div className="space-y-6">
                            <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                                <h3 className="text-2xl font-black text-slate-900 uppercase flex items-center gap-3">
                                    <i className="fas fa-layer-group text-blue-600"></i> Cross-Reference Matrix
                                </h3>
                                <button onClick={() => setSelectedComparisonCodes([])} className="px-4 py-2 bg-rose-50 text-rose-600 rounded-xl text-xs font-black uppercase border border-rose-100 hover:bg-rose-100 transition-all">{t('common_cancel')}</button>
                            </div>
                        ...
                        <div className="bg-white rounded-[40px] border-2 border-dashed border-slate-200 p-16 text-center shadow-inner">
                            ...
                            <h3 className="text-2xl font-black text-slate-800 uppercase mb-3">Compare 2-3 Packages</h3>
                            <p className="text-base text-slate-500 max-w-md mx-auto mb-8 leading-relaxed font-medium">Select up to 3 packages from the <strong>Refill Optimizer</strong> tab by clicking the checkbox on each card to view the cross-reference matrix here.</p>
                            <button onClick={() => setViewMode('OPTIMIZER')} className="px-8 py-3 bg-blue-600 text-white rounded-xl text-sm font-black uppercase shadow-lg shadow-blue-200 transition-all hover:bg-blue-700 active:scale-95">Go to Optimizer</button>
                        </div>
                    )}
                    ...
                    <div className="pt-10 border-t border-slate-200">
                        <h3 className="text-2xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                            <i className="fas fa-wand-magic-sparkles text-purple-600"></i> Similarity Suggestions (AI Insights)
                        </h3>
                                ...
                                <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 flex justify-between items-center shadow-sm hover:border-purple-300 hover:shadow-md transition-all group">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <div className="text-xs font-bold text-slate-400">{pair.sharedCount} shared</div>
                                        </div>
                ...
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] animate-fadeIn">
                    <div className="bg-slate-900/95 backdrop-blur-md text-white px-6 py-3.5 rounded-3xl shadow-2xl flex items-center gap-6 border border-white/10">
                        <div className="flex flex-col border-r border-white/10 pr-6 shrink-0">
                            <span className="text-xs font-black text-slate-400 uppercase tracking-widest mb-0.5">{t('common_selected')}</span>
                            <div className="text-base font-black text-blue-400">{selectedComparisonCodes.length} / 3</div>
                        </div>
                        ...
                        {selectedComparisonCodes.length >= 2 && (
                            ...
                            >
                                <i className="fas fa-chart-simple"></i> Compare
                            </button>
                        )}
                    </div>
                </div>
