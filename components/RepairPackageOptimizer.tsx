    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {metrics.map(m => (
                <div
                    key={m.label}
                    className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-3 shadow-sm"
                >
...
const ComparisonMatrix = ({
    ...
    selectedSets: any[];
    onItemSelect: (i: InventoryItem) => void;
}) => {
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="p-4 min-w-[280px] text-xs font-black text-slate-400 uppercase tracking-widest border-r border-slate-200">
                                Linh kiện / Phụ tùng tổng hợp
                            </th>
                            {selectedSets.map((set, idx) => (
                                <th
                                    key={set.setCode}
                                    className={`p-4 min-w-[220px] border-r border-slate-200 last:border-r-0 ${idx === 0 ? 'bg-blue-50/30' : idx === 1 ? 'bg-amber-50/30' : 'bg-purple-50/30'}`}
                                >
                                    <div className="text-xs font-black text-slate-400 uppercase mb-1">
                                        Gói {String.fromCharCode(65 + idx)}
                                    </div>
                                    <div
                                        className="text-sm font-black text-slate-800 uppercase truncate"
                                        title={set.setName}
                                    >
                                        {set.setName}
                                    </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {allSkus.map(sku => {
                            ...
                            return (
                                <tr
                                    key={sku}
                                    className={`hover:bg-slate-50/80 transition-colors ${isCommon ? 'bg-emerald-50/20' : ''}`}
                                >
                                        ...
                                        <div className="flex items-center gap-2">
                                            {isCommon && (
                                                <div
                                                    ...
                                                    title="Linh kiện chung"
                                                ></div>
                                            )}
                                            <span
                                                className="font-mono text-sm font-black text-slate-700 cursor-pointer hover:text-blue-600"
                                                onClick={() => compData.item && onItemSelect(compData.item)}
                                            >
                                                {sku}
                                            </span>
                                        </div>
                                                ...
                                                {c ? (
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <div className="flex flex-col text-right">
                                                            ...
                                                            <span
                                                                className={`text-sm font-black ${c.currentStock < c.def.QtyRequired ? 'text-rose-600' : 'text-emerald-600'}`}
                                                            >
                                                                {c.currentStock}
                                                            </span>
                                                        </div>
                    ...
                    <tfoot className="bg-slate-50 font-black border-t-2 border-slate-200">
                        <tr>
                            <td className="p-4 text-xs text-slate-500 uppercase border-r border-slate-200">
                                Hiệu quả tài chính
                            </td>
                            {selectedSets.map(set => (
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                            ...
                                            <span className="text-blue-700">
                                                {(set.packageStockValue / 1000000).toFixed(1)}tr
                                            </span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                            <span className="text-slate-400 uppercase">Sẵn có:</span>
                                            <span className="text-emerald-600 font-black">
                                                {set.maxSetsNB + set.maxSetsBB} bộ
                                            </span>
                                        </div>
                                    </div>
                                </td>
...
export const RepairPackageOptimizer = ({
    ...
    const [selectedComparisonCodes, setSelectedComparisonCodes] = useState<string[]>(
        initialState?.selectedComparisonCodes || [],
    );
    ...
    const handleSaveToCloud = async () => {
        const pin = prompt(
            'Vui lòng nhập Mã Phê Duyệt (Admin PIN) để lưu cấu hình Gói Phụ Tùng lên máy chủ:\n(Mặc định: 2026)',
        );
        if (pin === null) return;
    ...
    const { calculatedSets, itemMap } = useMemo(() => {
        ...
        groupedSets.forEach((components, setCode) => {
            ...
            sets.push({
                ...
                maxSetsBB,
                completionRateNB:
                    compResults.reduce((acc, c) => acc + Math.min(c.stockNB / c.def.QtyRequired, 1), 0) /
                    compResults.length,
                completionRateBB:
                    compResults.reduce((acc, c) => acc + Math.min(c.stockBB / c.def.QtyRequired, 1), 0) /
                    compResults.length,
    ...
    const manualComparisonSets = useMemo(
        () => calculatedSets.filter(s => selectedComparisonCodes.includes(s.setCode)),
        [calculatedSets, selectedComparisonCodes],
    );
    ...
    const handleAddToDraft = (set: any, region: 'NB' | 'BB') => {
        ...
        set.components.forEach((comp: any) => {
            ...
            if (shortage > 0) {
                ...
                if (transferQty > 0 || purchaseQty > 0) {
                    ...
                    currentNotes[comp.def.ItemCode] = currentNotes[comp.def.ItemCode]
                        ? `${currentNotes[comp.def.ItemCode]}, ${fullNote}`
                        : fullNote;
                }
            }
        });
        ...
        if (messages.length > 0) {
            ...
            alert(
                `Đã cập nhật dự thảo: ${messages.join(' và ')} cho ${qtyToBuild} bộ ${set.setName} (Khu vực ${region}).`,
            );
        } else {
        ...
        setOrderInputs(prev => ({
            ...prev,
            [set.setCode]: { ...prev[set.setCode], [region === 'NB' ? 'nb' : 'bb']: 0 },
        }));
    };
    ...
    const handleClearDraft = () => {
        ...
        const hasContent =
            Object.keys(draftData?.quantities || {}).length > 0 || Object.keys(draftData?.notes || {}).length > 0;
                ...
                {viewMode === 'AFFINITY' ? (
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[400px] bg-white rounded-2xl border-2 border-dashed border-slate-300 p-8 text-center">
                        <div className="w-20 h-20 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center mb-6">
                            <FaIcon className="fas fa-boxes-packing text-3xl" />
                        </div>
                        <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-2">
                            Chưa có dữ liệu Gói phụ tùng
                        </h3>
                        <p className="text-slate-500 max-w-md mb-8 text-base">
                            Vui lòng tải lên file định nghĩa (CSV) để hệ thống tính toán khả năng sẵn sàng của các bộ
                            linh kiện sửa chữa.
                        </p>
                        <input
                            type="file"
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl font-black uppercase tracking-widest shadow-lg shadow-blue-200 transition-all flex items-center gap-3"
                            >
                                <FaIcon className="fas fa-upload" /> {t('kit_upload_btn')}
                            </button>
    ...
    return (
        <div className="flex flex-col h-full space-y-6 relative pb-24">
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden border border-white/5 shadow-premium">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-amber-200 text-sm font-bold mt-1 uppercase tracking-widest">
                            Tối ưu hóa khả năng sẵn sàng của các bộ linh kiện sửa chữa
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 md:gap-3">
                        {(totalDraftItems > 0 || hasInputs) && (
                            <div className="flex items-center gap-2 bg-white px-2 py-1.5 rounded-xl border border-slate-200">
                                <span className="text-[10px] font-black text-slate-400 uppercase pl-1">
                                    Đang chọn: {totalDraftItems} mã
                                </span>
                                <button
                        ...
                        >
                            <FaIcon
                                className={`fas ${isLoadingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-down'} text-blue-600`}
                            />{' '}
                            Tải Cloud
                        </button>
                        ...
                        {kittingDefs.length > 0 && (
                            ...
                            >
                                <FaIcon
                                    className={`fas ${isSavingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-up'} text-emerald-600`}
                                />{' '}
                                Lưu Cloud
                            </button>
                        )}
                        ...
                        <div className="relative group w-56">
                            ...
                            <input
                                type="text"
                                placeholder="Tìm Gói / Model xe..."
                                value={searchTerm}
                                onChange={e => setSearchTerm(e.target.value)}
                                className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl outline-none text-xs font-black text-slate-900 placeholder-slate-400 focus:ring-4 focus:ring-blue-500/5 transition-all"
                            />
                        </div>
                    </div>
                </div>
            ...
            {viewMode === 'OPTIMIZER' && !searchTerm && <RepairPackageMetrics sets={calculatedSets} />}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-fadeIn">
                    {filteredSets.map((set: any) => {
                        ...
                        return (
                            <div
                                key={set.setCode}
                                className={`bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition-all flex flex-col relative ${selectedComparisonCodes.includes(set.setCode) ? 'border-blue-500 ring-2 ring-blue-50 bg-blue-50/10' : 'border-slate-200'}`}
                            >
                                ...
                                <div className="flex justify-between items-start mb-4 border-b border-slate-50 pb-4 pr-8">
                                    <div>
                                        <h3 className="font-black text-slate-800 text-base uppercase truncate max-w-[220px]">
                                            {set.setName}
                                        </h3>
                                        <div className="flex items-center gap-2 mt-1">
                                            <div className="text-xs text-slate-400 font-mono font-bold bg-slate-100 px-1.5 rounded">
                                                {set.setCode}
                                            </div>
                                            {set.modelCar && (
                                                <div className="text-2xs font-black text-white bg-slate-400 px-1.5 rounded uppercase">
                                                    {set.modelCar}
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="mt-3 flex gap-1 items-center">
                                            <div className="relative w-full">
                                                <input
                                                    ...
                                                    min="0"
                                                    placeholder="SL Đặt"
                                                    className="w-full h-8 pl-2 pr-8 text-center text-sm font-black bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-400 transition-colors"
                                                    value={orderInputs[set.setCode]?.nb || ''}
                                                    onChange={e =>
                                                        setOrderInputs(p => ({
                                                            ...p,
                                                            [set.setCode]: {
                                                                ...p[set.setCode],
                                                                nb: parseInt(e.target.value) || 0,
                                                            },
                                                        }))
                                                    }
                                                />
                                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-2xs text-slate-400 font-bold uppercase">
                                                    Bộ
                                                </span>
                                            </div>
                                            <button
                                                onClick={() => handleAddToDraft(set, 'NB')}
                                                className="bg-blue-600 text-white w-8 h-8 rounded-lg flex items-center justify-center shadow-sm active:scale-90 transition-all hover:bg-blue-700"
                                                title="Thêm gói vào dự thảo (NB)"
                                            >
                                                <FaIcon className="fas fa-cart-plus text-xs" />
                                            </button>
                                        </div>
                                    </div>
                                    {/* MIỀN BẮC REFILL */}
                                        ...
                                        <div className="mt-3 flex gap-1 items-center">
                                            <div className="relative w-full">
                                                <input
                                                    ...
                                                    min="0"
                                                    placeholder="SL Đặt"
                                                    className="w-full h-8 pl-2 pr-8 text-center text-sm font-black bg-white border border-slate-200 rounded-lg outline-none focus:border-amber-400 transition-colors"
                                                    value={orderInputs[set.setCode]?.bb || ''}
                                                    onChange={e =>
                                                        setOrderInputs(p => ({
                                                            ...p,
                                                            [set.setCode]: {
                                                                ...p[set.setCode],
                                                                bb: parseInt(e.target.value) || 0,
                                                            },
                                                        }))
                                                    }
                                                />
                                            ...
                                            <button
                                                onClick={() => handleAddToDraft(set, 'BB')}
                                                className="bg-blue-600 text-white w-8 h-8 rounded-lg flex items-center justify-center shadow-sm active:scale-90 transition-all hover:bg-blue-700"
                                                title="Thêm gói vào dự thảo (BB)"
                                            >
                                                <FaIcon className="fas fa-cart-plus text-xs" />
                                            </button>
                                        </div>
                                ...
                                <button
                                    onClick={() => setExpandedSet(expandedSet === set.setCode ? null : set.setCode)}
                                    className="mt-4 text-xs font-black text-slate-400 hover:text-blue-600 text-center uppercase tracking-[0.2em] transition-colors"
                                >
                                        ...
                                        <table className="w-full text-xs text-left">
                                            <thead className="bg-slate-50 text-slate-400 font-black uppercase">
                                                <tr>
                                                    ...
                                                    <th className="px-2 py-1.5 text-center bg-blue-50 text-blue-700">
                                                        SL Cần (Input)
                                                    </th>
                                                    <th className="px-2 py-1.5 text-right">Tồn NB</th>
                                                    <th className="px-2 py-1.5 text-right">Tồn BB</th>
                                                    <th className="px-2 py-1.5 text-center bg-slate-100 text-slate-700">
                                                        Giải Pháp
                                                    </th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-50">
                                                {set.components.map((c: any) => {
                                                    ...
                                                    const transferToBB = Math.min(
                                                        shortageBB,
                                                        Math.max(0, c.stockNB - qtyNeededNB),
                                                    ); // Can take from remaining NB?
                                                    ...
                                                    return (
                                                        <tr
                                                            ...
                                                            onClick={() => onItemSelect(itemMap.get(c.def.ItemCode)!)}
                                                        >
                                                            <td className="px-2 py-1.5 font-bold text-slate-700 font-mono">
                                                                {c.def.ItemCode}
                                                            </td>
                                                            ...
                                                            <td className="px-2 py-1.5 text-center bg-blue-50/30 font-black text-blue-700">
                                                            ...
                                                            <td className="px-2 py-1.5 text-center bg-slate-50/50 border-x border-slate-100">
                                                                {totalNeeded > 0 ? (
                                                                    <div className="flex justify-center gap-2 text-2xs font-bold">
                                                                        {totalStockUse > 0 && (
                                                                            <span
                                                                                className="text-emerald-600 bg-emerald-50 px-1.5 rounded border border-emerald-100"
                                                                                title="Dùng tồn kho"
                                                                            >
                                                                                ✅ {totalStockUse}
                                                                            </span>
                                                                        ...
                                                                        {totalTransfer > 0 && (
                                                                            <span
                                                                                className="text-purple-600 bg-purple-50 px-1.5 rounded border border-purple-100"
                                                                                title={`Điều chuyển: ${transferToNB > 0 ? 'BB->NB' : 'NB->BB'}`}
                                                                            >
                                                                                🔄 {totalTransfer}
                                                                            </span>
                                                                        )}
                                                                        {totalBuy > 0 && (
                                                                            <span
                                                                                className="text-rose-600 bg-rose-50 px-1.5 rounded border border-rose-100"
                                                                                title="Đặt mua mới"
                                                                            >
                                                                                🚢 {totalBuy}
                                                                            </span>
                ...
                <div className="space-y-8 animate-fadeIn">
                    ...
                    {selectedComparisonCodes.length >= 2 ? (
                        <div className="space-y-6">
                            <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                                <h3 className="text-2xl font-black text-slate-900 uppercase flex items-center gap-3">
                                    <FaIcon className="fas fa-layer-group text-blue-600" /> Ma trận đối ứng linh kiện đã
                                    chọn
                                </h3>
                                <button
                                    onClick={() => setSelectedComparisonCodes([])}
                                    className="px-4 py-2 bg-rose-50 text-rose-600 rounded-xl text-xs font-black uppercase border border-rose-100 hover:bg-rose-100 transition-all"
                                >
                                    Hủy so sánh
                                </button>
                            </div>
                        ...
                        <div className="bg-white rounded-[40px] border-2 border-dashed border-slate-200 p-16 text-center shadow-inner">
                            ...
                            <h3 className="text-2xl font-black text-slate-800 uppercase mb-3">
                                So sánh tùy chọn 2-3 gói
                            </h3>
                            <p className="text-base text-slate-500 max-w-md mx-auto mb-8 leading-relaxed font-medium">
                                Bạn có thể chọn tối đa 3 gói từ tab <strong>Refill Optimizer</strong> bằng cách tích
                                chọn vào nút ở góc phải mỗi thẻ gói để xem bảng đối ứng linh kiện chi tiết tại đây.
                            </p>
                            <button
                                onClick={() => setViewMode('OPTIMIZER')}
                                className="px-8 py-3 bg-blue-600 text-white rounded-xl text-sm font-black uppercase shadow-lg shadow-blue-200 transition-all hover:bg-blue-700 active:scale-95"
                            >
                                Đi tới chọn gói
                            </button>
                        </div>
                    ...
                    <div className="pt-10 border-t border-slate-200">
                        <h3 className="text-2xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                            <FaIcon className="fas fa-wand-magic-sparkles text-purple-600" /> Gợi ý các gói tương đồng
                            (AI Insights)
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {similarityPairs.map((pair: any, idx: number) => (
                                <div
                                    key={idx}
                                    className="bg-white p-4 rounded-2xl border border-slate-200 flex justify-between items-center shadow-sm hover:border-purple-300 hover:shadow-md transition-all group"
                                >
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            <div className="text-xs font-black text-purple-600 bg-purple-50 px-2.5 py-0.5 rounded-full border border-purple-100">
                                                {(pair.jaccardIndex * 100).toFixed(0)}% Overlap
                                            </div>
                                            <div className="text-xs font-bold text-slate-400">
                                                Chung {pair.sharedCount} mã
                                            </div>
                                        </div>
                                    ...
                                    <button
                                        onClick={() => {
                                            setSelectedComparisonCodes([pair.setA.setCode, pair.setB.setCode]);
                ...
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] animate-fadeIn">
                    <div className="bg-slate-900/95 backdrop-blur-md text-white px-6 py-3.5 rounded-3xl shadow-2xl flex items-center gap-6 border border-white/10">
                        <div className="flex flex-col border-r border-white/10 pr-6 shrink-0">
                            <span className="text-xs font-black text-slate-400 uppercase tracking-widest mb-0.5">
                                So sánh đã chọn
                            </span>
                            <div className="text-base font-black text-blue-400">
                                {selectedComparisonCodes.length} / 3 Gói
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2 overflow-x-auto max-w-[300px] no-scrollbar">
                            {selectedComparisonCodes.map(code => (
                                <div
                                    key={code}
                                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 border border-white/5 group"
                                >
