...
export const MetricCard = ({ label, value, subValue, icon, color = 'slate', onClick, isActive, trend, badge, badgeColor }: MetricCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-3">
        ...
        <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center text-base text-white ${style.iconBg} transition-all group-hover:scale-110 group-hover:rotate-12 duration-300 backdrop-blur-sm`}>
          <i className={`fas ${icon}`}></i>
        </div>

        {/* Content */}
        <div className="flex-1 min-w-0">
          <Typography variant="label" className={`${style.labelText} metric-label`}>
            {label}
          </Typography>
          <Typography variant="h2" className={`tracking-tight leading-snug ${style.valueText}`}>
            {value}
          </Typography>
          <Typography variant="body-sm" className={`truncate ${style.subText} trend-value`}>
            {subValue}
          </Typography>
        </div>
        ...
        {badge ? (
          ...
          <div className={`px-2 py-1 rounded-lg flex items-center gap-1 bg-white/15 border border-white/20 backdrop-blur-sm flex-shrink-0 ${trend.direction === 'up' ? 'text-emerald-200' :
            trend.direction === 'down' ? 'text-rose-200' : 'text-white/70'
            }`}>
            {(trend.direction === 'up' || trend.direction === 'down') && (
              <i className={`fas fa-arrow-${trend.direction} text-2xs`}></i>
            )}
            <Typography variant="label" className="inherit-color">
              {trend.value}
            </Typography>
          </div>
        )}
