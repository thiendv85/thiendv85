...
export const SupersessionCSVUpload: React.FC<SupersessionCSVUploadProps> = ({
    ...
    if (status === 'IDLE') {
        return (
            ...
            >
                ...
                <div
                    className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${dragActive ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}
                >
                    <Upload size={32} />
                </div>
                ...
                <p className="text-xs text-slate-500 max-w-xs">{t('ss_upload_desc')} (Max 5MB)</p>

                <div className="mt-6 flex items-center gap-2" onClick={e => e.stopPropagation()}>
                    <button
                        onClick={() => inputRef.current?.click()}
                        className="px-6 py-2 bg-white border border-slate-200 rounded-xl text-xs font-black text-slate-600 shadow-sm hover:text-blue-600 hover:border-blue-200 transition-all"
                    >
                        Browse File
                    </button>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
                ...
                <div className="p-6">
                    ...
                    <div className="flex gap-3 justify-end">
                        <button
                            onClick={handleReset}
                            className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-xs font-black uppercase hover:bg-slate-50 transition-all"
                        >
                            Cancel
                        </button>
                        <button
                            onClick={handleConfirm}
                            className="px-6 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-black uppercase shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all flex items-center gap-2"
                        >
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
                <div className="bg-emerald-50 px-6 py-8 text-center border-b border-emerald-100">
                    ...
                    <p className="text-emerald-600 text-sm font-bold mt-1">
                        Ready to import {mappings.length} mappings
                    </p>
                </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-slate-100">
                    ...
                    <div
                        className={`p-4 rounded-xl border text-center ${errorCount > 0 ? 'bg-rose-50 border-rose-100' : 'bg-slate-50 border-slate-100'}`}
                    >
                        <div
                            className={`text-2xs font-black uppercase tracking-widest ${errorCount > 0 ? 'text-rose-400' : 'text-slate-400'}`}
                        >
                            Critical Errors
                        </div>
                        <div
                            className={`text-2xl font-black mt-1 ${errorCount > 0 ? 'text-rose-600' : 'text-slate-400'}`}
                        >
                            {errorCount}
                        </div>
                    </div>
                    <div
                        className={`p-4 rounded-xl border text-center ${warningCount > 0 ? 'bg-amber-50 border-amber-100' : 'bg-slate-50 border-slate-100'}`}
                    >
                        <div
                            className={`text-2xs font-black uppercase tracking-widest ${warningCount > 0 ? 'text-amber-500' : 'text-slate-400'}`}
                        >
                            Warnings
                        </div>
                        <div
                            className={`text-2xl font-black mt-1 ${warningCount > 0 ? 'text-amber-600' : 'text-slate-400'}`}
                        >
                            {warningCount}
                        </div>
                    </div>
                    ...
                    <div className="p-6 bg-slate-50/50 max-h-60 overflow-y-auto custom-scrollbar">
                        ...
                        <ul className="space-y-2">
                            {graphResult.validationErrors.map((err, i) => (
                                <li
                                    key={`e-${i}`}
                                    className="flex items-start gap-2 text-xs font-bold text-rose-600 bg-white p-2 rounded border border-rose-100"
                                >
                            ...
                            {graphResult.validationWarnings.map((warn, i) => (
                                <li
                                    key={`w-${i}`}
                                    className="flex items-start gap-2 text-xs font-medium text-amber-700 bg-white p-2 rounded border border-amber-100"
                                >
                ...
                <div className="p-6 flex justify-end gap-3 bg-white">
                    ...
                    <button
                        ...
                        className="px-8 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-black uppercase shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all flex items-center gap-2"
                    >
    ...
    return (
        <div className="bg-white rounded-2xl border border-rose-200 p-8 text-center animate-fadeIn">
            ...
            <button
                onClick={handleReset}
                className="px-6 py-2 bg-white border border-slate-200 hover:border-rose-300 text-slate-700 rounded-xl font-bold text-xs uppercase tracking-wide"
            >
                Try Again
            </button>
        </div>
