      ...
      switch (block.type) {
        case 'header':
          return block.lines.map((line, lIdx) => (
            <h4 key={`h-${bIdx}-${lIdx}`} className="font-black text-emerald-950 mt-6 mb-3 text-base flex items-center gap-2 border-b-2 border-emerald-100 pb-2 uppercase" 
          ...
          return (
            <ul key={`l-${bIdx}`} className="space-y-2 my-3 ml-2">
              {block.lines.map((line, lIdx) => {
                ...
                return (
                  <li key={lIdx} className="flex items-start gap-3 bg-white/50 p-1.5 rounded-lg border border-transparent hover:border-emerald-100 transition-colors">
                    <span className="text-lg">{icon}</span>
                    <p className="flex-1 leading-relaxed text-sm" dangerouslySetInnerHTML={{ __html: formatText(content) }} />
                  </li>
                );
              })}
            </ul>
        ...
        case 'table':
          const rows = block.lines.filter(l => !l.includes('---')).map(l => l.split('|').filter(cell => cell.trim() !== '').map(cell => cell.trim()));
          if (rows.length === 0) return null;
          const [thead, ...tbody] = rows;
          return (
            <div key={`t-${bIdx}`} className="my-4 overflow-hidden border border-emerald-200 rounded-xl shadow-sm">
              <table className="w-full text-left border-collapse">
                <thead className="bg-emerald-600 text-white"><tr>{thead.map((cell, i) => <th key={i} className="p-2 text-xs uppercase font-black" dangerouslySetInnerHTML={{ __html: formatText(cell) }} />)}</tr></thead>
                <tbody className="bg-white">{tbody.map((row, i) => (<tr key={i} className="border-t border-emerald-100 hover:bg-emerald-50/50 transition-colors">{row.map((cell, j) => <td key={j} className="p-2 text-xs" dangerouslySetInnerHTML={{ __html: formatText(cell) }} />)}</tr>))}</tbody>
              </table>
            </div>
          );
        default:
          return block.lines.map((line, lIdx) => (
            <p key={`p-${bIdx}-${lIdx}`} className="mb-2 text-gray-700 leading-relaxed text-sm" dangerouslySetInnerHTML={{ __html: formatText(line) }} />
          ));
      }
    });
  };
      ...
      >
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className={`relative inline-flex rounded-full h-2 w-2 ${isLoading ? 'bg-emerald-500' : (isInitialState ? 'bg-slate-500' : 'bg-green-500')}`}></span>
