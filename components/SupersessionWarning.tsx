    ...
    return (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center animate-fadeIn my-2">
            ...
            <div className="flex-1">
                ...
                <div className="text-xs text-amber-700 space-y-1">
                    <p className="flex items-center gap-2">
                        {t('ss_warn_old')} <span className="font-mono font-bold bg-white px-1.5 py-0.5 rounded border border-amber-100">{partNumber}</span>
                        <ArrowRight size={12} />
                        {t('ss_warn_new')} <span className="font-mono font-bold bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded border border-emerald-100">{chain.currentPart}</span>
                    </p>
            ...
            <div className="flex items-center gap-2 shrink-0 w-full md:w-auto">
                ...
                >
                    <Check size={14} /> {t('ss_warn_use_new')}
                </button>
                <button
