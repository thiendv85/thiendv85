...
const ACTION_CONFIG = {
    approved: { title: 'Approve Adjusted Order', color: 'bg-emerald-600', icon: 'fa-circle-check', btn: 'Confirm & Approve' },
    rejected: { title: 'Reject Draft Completely', color: 'bg-rose-600', icon: 'fa-circle-xmark', btn: 'Confirm Rejection' },
    returned: { title: 'Return for Revision', color: 'bg-indigo-600', icon: 'fa-rotate-left', btn: 'Confirm Return' },
};
            ...
            <div className="relative w-full max-w-[600px] bg-white rounded-[40px] shadow-2xl overflow-hidden animate-slideUp flex flex-col">
                ...
                <div className={`${config.color} p-8 text-white flex items-center gap-4`}>
                    ...
                    <div>
                        ...
                        <div className="text-white/70 text-xs font-bold uppercase tracking-widest mt-1">Final Controller Checkpoint</div>
                    </div>
                </div>
                ...
                <div className="p-10 space-y-8 flex-1 overflow-auto max-h-[70vh]">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                            <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Scope of Decision</div>
                        ...
                        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 text-right">
                            ...
                            <div className="text-2xl font-black text-slate-900 tracking-tight">
                                {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND', maximumFractionDigits: 0 }).format(summary.metrics.value.after)}
                            </div>
                            <div className={`text-[10px] font-black uppercase mt-1 ${summary.metrics.value.delta > 0 ? 'text-blue-600' : 'text-slate-400'}`}>
                                Delta: {summary.metrics.value.delta > 0 ? '+' : ''}{Math.round(summary.metrics.value.deltaPct * 100)}% from draft
                            </div>
                        </div>
                    </div>
                    ...
                    {reasons.length > 0 && (
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Primary Business Reason</label>
                    ...
                    {/* Manager Comments */}
                    <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Executive Comments / Notes</label>
                        <textarea 
                            value={comment}
                            onChange={(e) => setComment(e.target.value)}
                            placeholder={action === 'approved' ? 'Add any specific notes for this approval (Optional)...' : 'Briefly explain your decision logic...'}
                ...
                <div className="px-10 py-8 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
                    <button onClick={onCancel} className="text-slate-400 font-black text-xs uppercase tracking-widest hover:text-slate-600 transition-all">
                        Cancel Action
                    </button>
                    <button 
                        onClick={() => onConfirm(comment, selectedReason)}
                        disabled={reasons.length > 0 && !selectedReason}
                        className={`px-10 py-4 ${config.color} text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl transition-all active:scale-95 disabled:grayscale disabled:opacity-50 hover:shadow-2xl`}
                    >
                        {config.btn}
                    </button>
                </div>
