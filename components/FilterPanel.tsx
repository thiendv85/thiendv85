...
const SpecialFilterButton = ({
    ...
    <button
        ...
        className={`lg-icon-btn outline-none ${isActive ? 'lg-active' : ''}`}
    >
...
export const FilterPanel = React.memo(
    ({ data, settings, onSettingsChange, filters, onFiltersChange, sourceName, loisProfiles }: FilterPanelProps) => {
        const { t } = useLanguage();
        ...
        const trendFlags = useMemo(
            () => Array.from(new Set(data.map(item => item.TrendFlag).filter(Boolean))).sort(),
            [data],
        );
        const uniqueSources = useMemo(() => {
            return Array.from(new Set(data.map(i => `${i.BrandName || ''}|${i.SourceId || ''}`)))
                ...
                .map(key => {
                    ...
                    const profile = settings.sourceProfiles?.find(
                        p =>
        ...
        const getSelectClass = (isActive: boolean) => {
            const base =
                'w-full cursor-pointer px-3 py-1 bg-white border rounded-xl text-xs font-black focus:ring-4 focus:ring-blue-50 focus:border-blue-400 outline-none h-9 shadow-sm transition-all uppercase tracking-tighter appearance-none transition-all duration-200';
            return isActive
                ? `${base} border-blue-600 text-blue-800 bg-blue-50/50`
                : `${base} border-slate-200 text-slate-600 hover:border-blue-300 hover:text-blue-700`;
        };
        ...
        const toggleLois = (lois: string) => {
            ...
            const next = current.includes(lois) ? current.filter(id => id !== lois) : [...current, lois];
                    ...
                    <div className="lg:col-span-9 bg-white/40 backdrop-blur-md p-3 sm:p-4 rounded-3xl border border-white/50 shadow-sm">
                        ...
                        <div className="flex items-center justify-between pb-3 border-b border-slate-200/80">
                            <div className="lg-segmented">
                                {['All', 'P1', 'P2', 'P3'].map(p => (
                                    <button
                                        key={p}
                                        onClick={() => onFiltersChange({ ...filters, priority: p as any })}
                                        aria-pressed={filters.priority === p}
                                        className={filters.priority === p ? 'lg-active' : ''}
                                    >
                                        {p}
                                    </button>
                                ))}
                            ...
                            <div className="relative shrink-0 w-[140px]">
                                ...
                                >
                                    ...
                                    <FaIcon
                                        className={`fas fa-chevron-${isSourceDropdownOpen ? 'up' : 'down'} text-[10px] text-slate-500`}
                                    />
                                </button>
                                                    ...
                                                    <div
                                                        ...
                                                        onClick={() => {
                                                            const newSource = isSelected
                                                                ? (filters.source || []).filter(id => id !== s.id)
                                                                : [...(filters.source || []), s.id];
                                                            onFiltersChange({ ...filters, source: newSource });
                                                        }}
                                                    >
                                                        <div
                                                            className={`w-3.5 h-3.5 rounded-sm border flex items-center justify-center shrink-0 ${isSelected ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300'}`}
                                                        >
                                                            {isSelected && (
                                                                <FaIcon className="fas fa-check text-[9px]" />
                                                            )}
                                                        </div>
                                                        <span className="text-xs font-semibold text-slate-700 truncate uppercase">
                                                            {s.label}
                                                        </span>
                                                    </div>
                                                );
                        ...
                        <div className="flex items-center justify-between py-3 border-b border-slate-200/80">
                            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar flex-nowrap pr-2 py-1">
                                <button
                                    ...
                                    aria-pressed={(filters.lois || []).length === 0}
                                    className={`lg-pill outline-none shrink-0 ${(filters.lois || []).length === 0 ? 'lg-active' : ''}`}
                                >
                                    ALL
                                </button>
                                {loisGroups.map(g => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            aria-pressed={isActive}
                                            className={`lg-pill outline-none shrink-0 ${isActive ? 'lg-active' : ''}`}
                                        >
                                            {g}
                                        </button>
                                    );
                            ...
                            <div className="lg-segmented shrink-0">
                                <button
                                    onClick={() => onSettingsChange({ ...settings, costBasis: 'PP' })}
                                    aria-pressed={settings.costBasis === 'PP'}
                                    className={settings.costBasis === 'PP' ? 'lg-active' : ''}
                                >
                                    VND
                                </button>
                                <button
                                    onClick={() => onSettingsChange({ ...settings, costBasis: 'FOB' })}
                                    aria-pressed={settings.costBasis === 'FOB'}
                                    className={settings.costBasis === 'FOB' ? 'lg-active' : ''}
                                >
                                    EUR
                                </button>
                            </div>
                        ...
                        <div className="flex items-center justify-between py-3 border-b border-slate-200/80 flex-wrap gap-3">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div className="relative shrink-0 w-[110px]">
                                    <select
                                        ...
                                        className={`lg-select w-full cursor-pointer ${filters.status !== 'All' ? 'lg-active' : ''}`}
                                    >
                                ...
                                <div className="relative shrink-0 w-[130px]">
                                    <select
                                        ...
                                        onChange={e =>
                                            onFiltersChange({ ...filters, costRange: Number(e.target.value) })
                                        }
                                        data-active={filters.costRange > 0}
                                        className={`lg-select w-full cursor-pointer ${filters.costRange > 0 ? 'lg-active' : ''}`}
                                    >
                                        {COST_RANGES.map((r, i) => (
                                            <option key={i} value={i}>
                                                {r.label === 'TẤT CẢ (PP)' ? 'Cost Range' : r.label}
                                            </option>
                                        ))}
                                    </select>
                                    <FaIcon className="fas fa-chevron-down absolute right-3 bottom-[10px] text-[10px] text-slate-500 pointer-events-none" />
                                ...
                                <div className="relative shrink-0 w-[110px]">
                                    <select
                                        value={filters.trend}
                                        onChange={e => onFiltersChange({ ...filters, trend: e.target.value })}
                                        data-active={filters.trend !== 'All'}
                                        className={`lg-select w-full cursor-pointer ${filters.trend !== 'All' ? 'lg-active' : ''}`}
                                    >
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                                <SpecialFilterButton
                                    label={t('term_backorder')}
                                    icon="fa-radiation"
                                    isActive={filters.showBackorders}
                                    onClick={() =>
                                        onFiltersChange({ ...filters, showBackorders: !filters.showBackorders })
                                    }
                                />
                                <SpecialFilterButton
                                    label="Stockout"
                                    icon="fa-battery-empty"
                                    isActive={filters.specialFilter === 'critical_stockout'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter:
                                                filters.specialFilter === 'critical_stockout'
                                                    ? 'none'
                                                    : 'critical_stockout',
                                        })
                                    }
                                />
                                <SpecialFilterButton
                                    label={t('term_stockout')}
                                    icon="fa-battery-quarter"
                                    isActive={filters.specialFilter === 'stockout'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter: filters.specialFilter === 'stockout' ? 'none' : 'stockout',
                                        })
                                    }
                                />
                                <SpecialFilterButton
                                    ...
                                    isActive={filters.specialFilter === 'excess'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter: filters.specialFilter === 'excess' ? 'none' : 'excess',
                                        })
                                    }
                                />
                                <SpecialFilterButton
                                    label={t('term_has_po')}
                                    icon="fa-ship"
                                    isActive={filters.specialFilter === 'has_po'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter: filters.specialFilter === 'has_po' ? 'none' : 'has_po',
                                        })
                                    }
                                />
                                <SpecialFilterButton
                                    label={t('term_supersession')}
                                    icon="fa-exchange-alt"
                                    isActive={filters.specialFilter === 'has_supersession'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter:
                                                filters.specialFilter === 'has_supersession'
                                                    ? 'none'
                                                    : 'has_supersession',
                                        })
                                    }
                                ...
                                <SpecialFilterButton
                                    ...
                                    isActive={filters.specialFilter === 'has_seasonality'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter:
                                                filters.specialFilter === 'has_seasonality'
                                                    ? 'none'
                                                    : 'has_seasonality',
                                        })
                                    }
                                ...
                                <SpecialFilterButton
                                    ...
                                    isActive={filters.specialFilter === 'has_warning'}
                                    onClick={() =>
                                        onFiltersChange({
                                            ...filters,
                                            specialFilter:
                                                filters.specialFilter === 'has_warning' ? 'none' : 'has_warning',
                                        })
                                    }
                                />
                            </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                {DEBT_STATUS_OPTIONS.map(opt => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {isActive ? (
                                                <FaIcon className="fas fa-check mr-2 text-[10px] text-white/80" />
                                            ) : null}
                                            {opt.label
                                                .replace('Trạng thái Cung ứng (Debt): ', '')
                                                .replace('BÌNH THƯỜNG', 'Normal')
                                                .replace('TỒN ĐỦ TRẢ', 'Sufficient')
                                                .replace('TRẢ TRONG THÁNG', 'Repay')
                                                .replace('PO ĐỦ TRẢ', 'PO Cover')
                                                .replace('THIẾU (CÓ PO)', 'Short+PO')
                                                .replace('THIẾU (NO PO)', 'Short-NoPO')}
                                        </button>
                                    );
                                })}
                            </div>
                                ...
                                <button
                                    onClick={() => {
                                        const resetProfiles = settings.sourceProfiles?.map(p => {
                                            const def = DEFAULT_SOURCE_PROFILES.find(
                                                d => d.id.toUpperCase() === p.id.toUpperCase(),
                                            );
                                            return {
                                                ...p,
                                                lt: def?.lt ?? 90,
                        ...
                        <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-1 customer-scrollbar">
                            {(() => {
                                const uniqueSources = Array.from(
                                    new Set(data.map(i => `${i.BrandName || ''}|${i.SourceId || ''}`)),
                                )
                                    .filter(key => key !== '|')
                                    .map(key => {
                                        const [brand, sid] = key.split('|');
                                ...
                                return uniqueSources.map(src => {
                                    ...
                                    const brandFallback = DEFAULT_SOURCE_PROFILES.filter(
                                        d => d.brand.toLowerCase() === brandKey,
                                    ).reduce((max, d) => (d.lt > max.lt ? d : max), DEFAULT_SOURCE_PROFILES[0]);

                                    const valLT =
                                        profile?.lt ?? defaultProfile?.lt ?? brandFallback?.lt ?? settings.params.lt;
                                    const valSP =
                                        profile?.sp ?? defaultProfile?.sp ?? brandFallback?.sp ?? settings.params.sp;
                                    const valSSP =
                                        profile?.ssp ??
                                        defaultProfile?.ssp ??
                                        brandFallback?.ssp ??
                                        ...
                                        if (profile) {
                                            ...
                                            const newProfile: any = {
                                                ...
                                                ssp:
                                                    field === 'ssp'
                                                        ? value
                                                        : (defaultProfile?.ssp ?? settings.params.ssp),
                                            };
                                            onSettingsChange({
                                                ...settings,
                                    ...
                                    return (
                                        <div
                                            key={`${src.brand}-${src.sid}`}
                                            className="flex items-center justify-between gap-3 bg-white p-2.5 rounded-2xl border border-slate-200 transition-all group shadow-sm hover:border-blue-400"
                                        >
                                            <div className="flex flex-col min-w-0 pr-2">
                                                <span
                                                    className={`text-[10px] font-black uppercase leading-none truncate tracking-tight ${src.brand.toLowerCase() === 'kia' ? 'text-blue-800' : src.brand.toLowerCase() === 'mazda' ? 'text-emerald-800' : 'text-slate-800'}`}
                                                >
                                            ...
                                            <div className="flex gap-2 shrink-0">
                                                {[
                                                    ...
                                                    <div key={p.id} className="flex flex-col items-center gap-0.5">
                                                        <span className="text-[8px] font-black text-blue-700/60 uppercase tracking-tighter">
                                                            {p.label}
                                                        </span>
                                                        <input
                                                            type="number"
                ...
                <>
                    ...
                    <div className="flex items-center gap-2 bg-white rounded-2xl border border-slate-200 px-3 py-2.5 shadow-sm">
                        ...
                        >
                            ...
                            {activeFilterCount > 0 && (
                                <span className="bg-blue-600 text-white text-[9px] font-black min-w-[18px] h-[18px] rounded-full flex items-center justify-center px-1">
                                    {activeFilterCount}
                                </span>
                            )}
                        </button>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="p-4 border-t border-slate-200 bg-white rounded-b-3xl shrink-0">
                                        <button
                                            onClick={() => setMobileOpen(false)}
                                            className="lg-btn lg-btn-blue lg-btn-lg lg-btn-full"
                                        >
                                            Áp dụng bộ lọc
                                        </button>
                                    </div>
