                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <button onClick={handleSave} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl text-xs font-black uppercase hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/20">
                            <FaIcon className={`fas ${saved ? 'fa-check' : 'fa-floppy-disk'}`}  /> {saved ? 'Đã lưu!' : 'Lưu Máy này'}
                        </button>
                        <button onClick={handleLoadFromCloud} disabled={isLoadingCloud} className="flex items-center gap-2 bg-blue-500/30 border border-blue-400/30 text-blue-100 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-blue-500/50 transition-all shadow-lg shadow-blue-500/20">
                            <FaIcon className={`fas ${isLoadingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-down'}`}  /> Tải Cloud
                        </button>
                        <button onClick={handleSaveToCloud} disabled={isSavingCloud} className="flex items-center gap-2 bg-emerald-500/30 border border-emerald-400/30 text-emerald-100 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-emerald-500/50 transition-all shadow-lg shadow-emerald-500/20">
                            <FaIcon className={`fas ${isSavingCloud ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-up'}`}  /> Lưu Cloud
                        </button>
                                    ...
                                    {extra.map(profile => (
                                        <div key={profile.id} className="grid grid-cols-12 gap-2 items-center py-2 border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <div className="col-span-6 flex justify-end">
                                                {isAdmin && (
                                                    <button onClick={() => setDraft(prev => ({ ...prev, sourceProfiles: prev.sourceProfiles.filter(p => p.id !== profile.id) }))} className="text-rose-500 p-2"><FaIcon className="fas fa-trash"  /></button>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3">
                                <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold ${
                                    ...
                                    <FaIcon className={`fas ${monthlyCurrentDate ? 'fa-calendar-check text-emerald-500' : 'fa-triangle-exclamation text-amber-500'}`}  />
                ...
                <div className="max-w-[1800px] mx-auto flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center bg-white border border-slate-200 rounded-xl p-1 shadow-sm overflow-hidden">
                            ...
                            >
                                {isSavingCloud ? <FaIcon className="fas fa-spinner fa-spin"  /> : <FaIcon className="fas fa-cloud-arrow-up"  />}
