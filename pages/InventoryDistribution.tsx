...
export const InventoryDistribution: React.FC<InventoryDistributionProps> = ({
    ...
    const allItems = useMemo(() => {
        ...
        items.sort((a, b) => {
            switch (sortKey) {
                case 'value_desc':
                    return b.transferValue - a.transferValue;
                case 'qty_desc':
                    return b.roundedQty - a.roundedQty;
                case 'benefit_desc':
                    return (b.transfer.transferNetBenefit || 0) - (a.transfer.transferNetBenefit || 0);
                case 'mos_asc': {
                    const mosA = Math.min(
                        a.item.computed?.transfer?.mosNB ?? 99,
                        a.item.computed?.transfer?.mosBB ?? 99,
                    );
                    const mosB = Math.min(
                        b.item.computed?.transfer?.mosNB ?? 99,
                        b.item.computed?.transfer?.mosBB ?? 99,
                    );
                    return mosA - mosB;
                }
                case 'code':
    ...
    return (
        ...
        >
            ...
            <div className="px-6 lg:px-8 pt-5 pb-4" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <section className="bo-tile-feature" style={{ padding: '20px 24px 18px' }}>
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            >
                                <FaIcon
                                    ...
                                    style={{ color: 'var(--bo-accent)', fontSize: 14 }}
                                    aria-hidden="true"
                                />
                            </div>
                            <div>
                                <div className="bo-eyebrow" style={{ fontSize: 11, marginBottom: 2 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            >
                                <FaIcon
                                    className={`fas ${isSaving ? 'fa-spinner fa-spin' : 'fa-cloud-arrow-up'}`}
                                    style={{ fontSize: 12 }}
                                    aria-hidden="true"
                                />
                            ...
                            {lastSaved && (
                                <span style={{ fontSize: 12, color: 'rgba(244, 241, 235, 0.4)', fontWeight: 700 }}>
                                    <FaIcon
                                        ...
                                        style={{ color: 'var(--bo-ok)', marginRight: 4 }}
                                    />
                                    {lastSaved}
                                </span>
                            )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
                        {(
                            [
                                {
                                    label: 'Tổng SKU',
                                    value: fmtNum(stats.totalSKU),
                                    sub: `NB→BB ${nbToBb.skuCount} · BB→NB ${bbToNb.skuCount}`,
                                },
                                {
                                    label: 'Tổng SL',
                                    value: fmtNum(stats.totalQty),
                                    sub: `${fmtNum(nbToBb.totalQty)} + ${fmtNum(bbToNb.totalQty)}`,
                                },
                                {
                                    label: 'Tổng Giá Trị',
                                    value: `${formatVND(stats.totalValue)}₫`,
                                    sub: `NB→BB ${formatVND(nbToBb.totalValue)} · BB→NB ${formatVND(bbToNb.totalValue)}`,
                                    warn: true,
                                },
                                {
                                    label: 'Lợi Ích Ròng',
                                    value: `${formatVND(stats.totalNetBenefit)}₫`,
                                    sub: `MOS +${stats.avgMOSImprovement.toFixed(2)} · BO ${(stats.boCoverageRate * 100).toFixed(0)}%`,
                                    accent: true,
                                },
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        {[
                            {
                                id: 'NB_TO_BB' as const,
                                label: 'NB → BB',
                                count: nbToBb.skuCount,
                                dot: 'var(--bo-bronze-strong)',
                            },
                            {
                                id: 'BB_TO_NB' as const,
                                label: 'BB → NB',
                                count: bbToNb.skuCount,
                                dot: 'var(--bo-accent)',
                            },
                        ].map(d => {
                            ...
                            return (
                                <button
                                    key={d.id}
                                    onClick={() => {
                                        setDirFilter(isActive ? 'ALL' : d.id);
                                        setCurrentPage(1);
                                    }}
                                    style={{
                                        display: 'inline-flex',
                                        ...
                                        background: isActive
                                            ? `color-mix(in srgb, ${d.dot} 10%, transparent)`
                                            : 'transparent',
            ...
            {stats.top5ByValue.length > 0 && (
                <div className="px-6 lg:px-8 pb-3" style={{ display: 'flex', gap: 6, overflow: 'auto' }}>
                    ...
                    {stats.top5ByValue.map(fi => (
                        <button key={fi.item.ItemCode} onClick={() => onItemSelect(fi.item)} className="bo-chip">
                            <span style={{ fontFamily: 'var(--bo-font-mono)', fontWeight: 800, fontSize: 13 }}>
                                {fi.item.ItemCode}
                            </span>
                            <span
                                style={{
                                    ...
                                    borderRadius: 4,
                                    background:
                                        fi.direction === 'NB_TO_BB' ? 'var(--bo-bronze-soft)' : 'var(--bo-accent-soft)',
                                    color:
                                        fi.direction === 'NB_TO_BB'
                                            ? 'var(--bo-bronze-strong)'
                                            : 'var(--bo-accent-deep)',
                                }}
                            >
                    ...
                    <details style={{ fontSize: 13 }}>
                        ...
                        >
                            ...
                            {filterResult.excluded.length > 20 && (
                                <div style={{ color: 'var(--bo-ink-muted)', fontWeight: 700 }}>
                                    …và {filterResult.excluded.length - 20} SKU khác
                                </div>
                            )}
                        </div>
                    </details>
            ...
            <div className="flex-1 overflow-hidden flex flex-col px-6 lg:px-8 pb-6">
                <div
                    className="bo-data-table-shell flex-1 flex flex-col"
                    style={{
                        background: 'var(--bo-paper)',
                        border: '1px solid var(--bo-hairline)',
                ...
                >
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
                            ...
                            >
                                <span style={{ fontFamily: 'var(--bo-font-mono)' }}>
                                    {allItems.length.toLocaleString('vi-VN')}
                                </span>{' '}
                                kết quả
                            </span>
                        </div>
                                    ...
                                    <tr style={{ borderBottom: '1px solid var(--bo-hairline)' }}>
                                        <th style={{ ...thStyle(), width: 40, textAlign: 'center' }}>
                                            <input
                                                type="checkbox"
                                                checked={
                                                    selectedCodes.size > 0 && selectedCodes.size === allItems.length
                                                }
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <span style={{ color: 'var(--bo-ink-muted)', fontWeight: 700 }}>
                                Tổng:{' '}
                                <span style={{ fontFamily: 'var(--bo-font-mono)', color: 'var(--bo-ink)' }}>
                                    {allItems.length.toLocaleString('vi-VN')}
                                </span>{' '}
                                SKU
                            </span>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            {paginationRange(currentPage, totalPages).map((p, i) =>
                                p === '...' ? (
                                    <span key={`dot-${i}`} style={{ padding: '0 4px', color: 'var(--bo-ink-muted)' }}>
                                        …
                                    </span>
                                ) : (
                                    <PaginationBtn
...
function TransferRow({
    ...
    return (
        <tr
            ...
            onMouseEnter={e => {
                if (!isSelected) e.currentTarget.style.background = 'var(--bo-surface-sunken)';
            }}
            onMouseLeave={e => {
        ...
        >
            ...
            <td style={{ padding: '12px' }}>
                <div
                    style={{ fontFamily: 'var(--bo-font-mono)', fontWeight: 800, fontSize: 13, color: 'var(--bo-ink)' }}
                >
                    {item.ItemCode}
                </div>
                <div
            ...
            <td style={{ padding: '12px' }}>
                <div style={{ width: 140 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                        <span style={{ fontWeight: 700, color: 'var(--bo-ink-muted)' }}>
                            OH: {donorStock.toLocaleString()}
                        </span>
                    ...
                    {donorBO > 0 && (
                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--bo-critical)', marginTop: 4 }}>
                            <FaIcon className="fas fa-exclamation-triangle" style={{ fontSize: 10, marginRight: 4 }} />
                            BO: {donorBO}
                        </div>
                    )}
                </div>
            ...
            <td style={{ padding: '12px' }}>
                <div style={{ width: 140 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                        <span style={{ fontWeight: 700, color: 'var(--bo-ink-muted)' }}>
                            OH: {receiverStock.toLocaleString()}
                        </span>
                        <span style={{ fontWeight: 700, color: 'var(--bo-ok)' }}>+{fi.roundedQty}</span>
                    </div>
                    <MOSTransition
                        before={receiverMOSBefore}
                        after={t.receiverMOSAfterReceive}
                        label={`${receiverLabel} MOS`}
                    />
                    {receiverBO > 0 && (
                        <div
                            style={{
                        ...
                        >
                            {hasCritical ? (
                                <>
                                    <FaIcon className="fas fa-check-circle" style={{ fontSize: 10, marginRight: 4 }} />
                                    BO {receiverBO} covered
                                </>
                            ) : (
                                <>
                                    <FaIcon
            ...
            <td style={{ padding: '12px', textAlign: 'right' }}>
                <div style={{ fontFamily: 'var(--bo-font-mono)', fontWeight: 800, color: 'var(--bo-accent-deep)' }}>
                    {formatVND(fi.transferValue)}₫
                </div>
                <div
                    style={{
                        ...
                        {t.reasonCodes.includes('DONOR_RISK_AFTER') && (
                            <span
                                style={{
                                    ...
                                    padding: '2px 6px',
                                    borderRadius: 4,
                                    background: 'var(--bo-critical-soft)',
                                    color: 'var(--bo-critical)',
                                }}
                            >
                                DONOR RISK
                            </span>
    ...
    if (items.length === 0) {
        return (
            <div style={{ textAlign: 'center', padding: '64px 0' }}>
                <FaIcon
                    className="fas fa-shuffle"
                    style={{ fontSize: 48, color: 'var(--bo-hairline)', display: 'block', margin: '0 auto 16px' }}
                />
                        ...
                        <div style={{ padding: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span style={{ fontWeight: 700, color: 'var(--bo-critical)' }}>
                                        −{fi.roundedQty}
                                    </span>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <span
                                        style={{
                                            ...
                                            fontSize: 13,
                                            fontWeight: 800,
                                            color: donorMOSBefore < 1.5 ? 'var(--bo-critical)' : 'var(--bo-ink)',
                                        }}
                                    >
                                    ...
                                    <FaIcon
                                        className="fas fa-arrow-right"
                                        style={{ fontSize: 10, color: 'var(--bo-ink-muted)' }}
                                    />
                                    <span
                                        style={{
                                            fontFamily: 'var(--bo-font-mono)',
                                            ...
                                            fontWeight: 800,
                                            color:
                                                (t.donorMOSAfterGive ?? donorMOSBefore) < 1
                                                    ? 'var(--bo-critical)'
                                                    : (t.donorMOSAfterGive ?? donorMOSBefore) < 2
                                                      ? 'var(--bo-accent)'
                                                      : 'var(--bo-ok)',
                                        }}
                                    >
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <span
                                        style={{
                                            ...
                                            fontSize: 13,
                                            fontWeight: 800,
                                            color: receiverMOSBefore < 1.5 ? 'var(--bo-critical)' : 'var(--bo-ink)',
                                        }}
                                    >
                                    ...
                                    <FaIcon
                                        className="fas fa-arrow-right"
                                        style={{ fontSize: 10, color: 'var(--bo-ok)' }}
                                    />
                                    <span
                                        style={{
                                            fontFamily: 'var(--bo-font-mono)',
                                            ...
                                            fontWeight: 800,
                                            color:
                                                (t.receiverMOSAfterReceive ?? receiverMOSBefore) < 1
                                                    ? 'var(--bo-critical)'
                                                    : (t.receiverMOSAfterReceive ?? receiverMOSBefore) < 2
                                                      ? 'var(--bo-accent)'
                                                      : 'var(--bo-ok)',
                                        }}
                                    >
