...
async function enforceRetentionLimit(maxSnapshots = 30): Promise<void> {
    try {
        ...
        if (paths.length > 0) {
            const { error: storageErr } = await supabase.storage.from('inventory_snapshots').remove(paths);
            // Storage removal failure is non-critical — metadata delete still proceeds
        }

        if (ids.length > 0) {
            const { error: dbErr } = await supabase.from('snapshot_metadata').delete().in('id', ids);
...
export async function uploadSnapshot(
    ...
    try {
        ...
        const { error: uploadErr } = await supabase.storage.from('inventory_snapshots').upload(path, compressed, {
...
export async function listSnapshots(limit = 50, brandFilter?: string | null): Promise<SnapshotMetadataRow[]> {
    try {
        let query = supabase.from('snapshot_metadata').select('*, profiles:uploaded_by(full_name)');
...
export async function getStorageUsage(): Promise<{ usedBytes: number; count: number }> {
    try {
        const { data } = await supabase.from('snapshot_metadata').select('file_size_bytes');
...
export async function loadSnapshot(storagePath: string): Promise<InventoryItem[] | null> {
    try {
        const { data, error } = await supabase.storage.from('inventory_snapshots').download(storagePath);
...
export async function deleteSnapshot(id: string, storagePath: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        const { error: storageErr } = await supabase.storage.from('inventory_snapshots').remove([storagePath]);
        ...
        const { error: dbErr } = await supabase.from('snapshot_metadata').delete().eq('id', id);
