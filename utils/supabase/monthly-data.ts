 ...
 */
export async function loadLatestMonthlyData(
    lastUpdatedAt?: string | null,
): Promise<{ data: Record<string, any>; updatedAt: string; isUpToDate?: boolean } | null> {
    try {
 ...
 */
export async function loadSpecificMonthlyData(
    month: string,
): Promise<{ data: Record<string, any>; updatedAt: string } | null> {
    try {
...
export async function deleteMonthlyData(snapshotMonth: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        const { error: dErr } = await supabase.from('monthly_sku_data').delete().eq('snapshot_month', snapshotMonth);
        ...
        const { error: cErr } = await supabase.from('monthly_snapshots').delete().eq('snapshot_month', snapshotMonth);
