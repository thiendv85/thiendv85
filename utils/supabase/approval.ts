...
export async function submitApprovalRequest(
    ...
    try {
        ...
        if (error && isMissingSummaryColumn(error) && SUMMARY_COLUMN_AVAILABLE !== false) {
            ...
            ({ data, error } = await supabase.from('approval_requests').insert(baseRow).select('id').single());
        } else if (!error) {
...
export async function fetchMyRequests(userId: string): Promise<ApprovalRequest[]> {
    const colsWith =
        'id, draft_name, brand, workflow_id, current_level, status, submitted_by, submitted_at, version, deadline, summary';
    const colsNo =
        'id, draft_name, brand, workflow_id, current_level, status, submitted_by, submitted_at, version, deadline';
    if (SUMMARY_COLUMN_AVAILABLE !== false) {
        try {
            const out = await selectAllPaginated<ApprovalRequest>(
                (from, to) =>
                    supabase
                        .from('approval_requests')
                        .select(colsWith)
    ...
    return selectAllPaginated<ApprovalRequest>(
        (from, to) =>
            supabase
                .from('approval_requests')
                .select(colsNo)
...
export async function fetchPendingForApprover(_userId?: string, approvalLevels?: number[]): Promise<ApprovalRequest[]> {
    const colsWith =
        'id, draft_name, brand, workflow_id, current_level, status, submitted_by, submitted_at, version, deadline, summary';
    const colsNo =
        'id, draft_name, brand, workflow_id, current_level, status, submitted_by, submitted_at, version, deadline';
    ...
    if (SUMMARY_COLUMN_AVAILABLE !== false) {
        try {
            requests = await selectAllPaginated<ApprovalRequest>(
                (from, to) =>
                    supabase
                        .from('approval_requests')
                        .select(colsWith)
...
export async function processApprovalAction(
    ...
    const selectQuery =
        'id, workflow_id, current_level, status, version' +
        ((modifiedQuantities || modifiedNotes) && !providedSnapshotData ? ', snapshot_data' : '');
    ...
    if (modifiedQuantities || modifiedNotes) {
        ...
        const updatedSnap = {
            ...(snapData || {}),
            ...(modifiedQuantities ? { quantities: modifiedQuantities } : {}),
