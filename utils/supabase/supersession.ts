...
export async function uploadSupersessionFile(
    ...
    try {
        ...
        // ── Multi-brand append model (fix 2026-05-27) ──
        // Trước: delete().gte('created_at','1970-01-01') wipe TOÀN BỘ uploads → CASCADE
        // xoá mappings → upload Mazda sau cùng làm mất các thương hiệu khác.
        // Sau: KHÔNG delete uploads cũ. Mappings upsert qua (old_part,new_part) —
        // pair trùng được overwrite, pair brand khác giữ nguyên. Multi-brand safe.
        const { count: previousCount } = await supabase
            .from('supersession_mappings')
            .select('id', { count: 'exact', head: true });

        // Create new upload row (audit trail, không delete cũ)
        ...
        const dedupMap = new Map<
            string,
            { old_part: string; new_part: string; interchangeable: boolean; upload_id: string }
        >();
        for (const r of rows) {
...
export async function deleteSupersessionUpload(uploadId: string): Promise<boolean> {
    const { error } = await supabase.from('supersession_uploads').delete().eq('id', uploadId);
    if (error) {
        return false;
    }
    return true;
...
export async function loadAllSupersessionMappings(): Promise<
    { old_part: string; new_part: string; interchangeable: boolean }[]
> {
    return selectAllPaginated<{ old_part: string; new_part: string; interchangeable: boolean }>((from, to) =>
        supabase.from('supersession_mappings').select('old_part, new_part, interchangeable').range(from, to),
    );
}

export function dbMappingsToApp(
    rows: { old_part: string; new_part: string; interchangeable: boolean }[],
): SupersessionMapping[] {
    return rows.map(r => ({
        oldPart: r.old_part,
